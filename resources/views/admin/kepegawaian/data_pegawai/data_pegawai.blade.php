...
@section('content')
    <div class="container">
        <div class="flex justify-between">
            ...
            <a href="{{ route('pegawai-fungsional.create') }}">
                <button type="button"
                                    ...
                                    @foreach ($pegawai as $pg)
                                        <tr>
                                            <td
                                                ...
                                                {{-- <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                                {{ $pg->unit_kerja_has_jabatan_fungsional->unitkerja->name }}</td> --}}
                                             <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium ">
                                                <div class="flex justify-end">
                                                    {{-- <form
                                                        action="{{ route('pegawai-fungsional.delete', ['id' => $pg->id]) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
                                                    ...
                                                    <a href="
                                                    {{ route('data-pegawai.detail', ['id' => $pg->id]) }}
                                                    "
