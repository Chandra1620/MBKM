...
@section('content')
    <div class="container">
        <div class="flex justify-between">
            <p class="pb-2 font-bold">Unit Kerja</p>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">atasan
                                            langsung
                                        </th>
                                        <th scope="col"
                                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">atasan
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    @foreach ($unitkerja as $unit)
                                        <tr>
                                            ...
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                                @if ($unit->jabatan_berwenang_id != $unit->atasan_langsung_id)
                                                {{ $unit->atasanlangsung->name }}
                                                @else
                                                {{ $unit->jabatanberwenang->name }}

                                                @endif
                                            </td>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                                @if ($unit->jabatanberwenang)
                                                    {{$unit->jabatanberwenang->name}}
                                                @endif
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium ">
                                                <div class="flex justify-end">
                                                    <a href="{{ route('unit-kerja.detail', ['id' => $unit->id]) }}">
                                                        <button type="button"
                                                    ...
                                                    <a href="{{ route('unit-kerja.edit', ['id' => $unit->id]) }}">
                                                        <button type="button"
                                                            class="py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border-2 border-yellow-200 font-semibold text-yellow-500 hover:text-white hover:bg-yellow-500 hover:border-yellow-500 focus:outline-none focus:ring-2 focus:ring-yellow-200 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800">
                                                            Update
                                                        </button>
                                                    ...
                                                    <form action="{{ route('unit-kerja.delete', ['id' => $unit->id]) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
